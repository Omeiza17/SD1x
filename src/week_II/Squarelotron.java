...
class Squarelotron {
    ...
    */
    private void rotate(){
        ...
        for (int i = 0; i < size; i++) {
            temp[i] = this.squarelotron[i].clone();
        }

        for (int i = 0; i < size; i++) {
            for (int j = size - 1; j >= 0; j--) {
                this.squarelotron[i][size - j -1] = temp[j][i];
            }
        }
    ...
    */
    void rotateRight(int numberOfTurns){
        numberOfTurns = (numberOfTurns % 4 + 4) % 4; // handles all negative cases of numberOfTurns i.e turns = -1 ~
        // turn = 3
